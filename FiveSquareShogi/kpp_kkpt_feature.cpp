...
#undef Mochi
	...
	void kpp_kkpt_feat::init(const std::string& folderpath) {
		if (!allocated) {
			allocated = true;
		...
		{
			std::ifstream fs(folderpath + "/KK.bin", std::ios::binary);
			if (!fs) {
				std::cerr << "error:file(KK.bin) cannot open" << std::endl;
				return;
			}
		...
		bool fPiece = false, fKPP = false, fKKP = false, fKK = false;
		if (dynamicPieceScore) {
		...
		{
			std::ifstream fs(folderpath + "/fKK.bin", std::ios::binary);
			if (!fs) {
				std::cerr << "error:file(fKK.bin) cannot open" << std::endl;
				return;
			}
			else {
				auto end = (char*)KK + sizeof(KKEvalElementType1);
		...
		using iKKEvalElementType0 = iEvalElementType[SquareNum];
		using iKKEvalElementType1 = iKKEvalElementType0[SquareNum];
		...
		if(!fKK){
			...
			iKKEvalElementType0* iKK = new iKKEvalElementType0[SquareNum];
			auto end = (char*)KK + sizeof(KKEvalElementType1);
			for (auto it = (char*)KK; it < end; it += (1 << 30)) {
				size_t size = (it + (1 << 30) < end ? (1 << 30) : end - it);
				fs.read(it, size);
			...
			for (unsigned sk = 0; sk < SquareNum; sk++) {
				for (unsigned gk = 0; gk < SquareNum; gk++) {
					KK[sk][gk][0] = iKK[sk][gk][0];
					KK[sk][gk][1] = iKK[sk][gk][1];
				}
			}
			delete[] iKK;
		}
	...
	void kpp_kkpt_feat::save(const std::string& folderpath) {
#ifndef _EVAL_FLOAT
		if (dynamicPieceScore) {
		...
		{
			std::ofstream fs(folderpath + "/fKK.bin", std::ios::binary);
			if (!fs) {
				std::cerr << "error:file(fKK.bin) cannot open" << std::endl;
				return;
			}
		...
		}
		{
			...
			for (unsigned sk = 0; sk < SquareNum; sk++) {
				for (unsigned gk = 0; gk < SquareNum; gk++) {
					iKK[sk][gk][0] = KK[sk][gk][0];
					iKK[sk][gk][1] = KK[sk][gk][1];
				}
			}
			auto end = (char*)iKK + sizeof(iKKEvalElementType1);
			for (auto it = (char*)iKK; it < end; it += (1 << 30)) {
				size_t size = (it + (1 << 30) < end ? (1 << 30) : end - it);
				fs.write(it, size);
			}
			delete[] iKK;
	...
	EvalSum kpp_kkpt_feat::EvalFull(const Kyokumen& kyokumen, const EvalList& elist) {
		...
		for (int i = 0; i < EvalList::EvalListSize; ++i) {
			...
			sum.p[1] += KKP[skpos][gkpos][k0];
		}
		sum.p[1][0] += elist.material * FVScale;
		sum.p[2][0] = KK[skpos][gkpos][0]; sum.p[2][1] = KK[skpos][gkpos][1];
		return sum;
	}

	void kpp_kkpt_feat::set(const Kyokumen& kyokumen) {
		idlist.set(kyokumen);
	...
	void kpp_kkpt_feat::proceed(const Kyokumen& before, const Move& move) {
		...
		if (!isInside(from)) { //駒台から
			...
			const unsigned gkpos = before.gOuPos();
			{
				...
				for (int i = 0; i < EvalList::EvalListSize; i++) {
					...
					sum.p[0][0] -= pskpp[index0];
					sum.p[0][1] -= pgkpp[index1];
				}
				sum.p[1] -= KKP[skpos][gkpos][bIndex0];
			}
			...
			Replace(bIndex1, aIndex1, idlist.list1);
			{
				...
				sum.p[1] += KKP[skpos][gkpos][aIndex0];
			}
		}
		else {
			...
			if (k == Koma::s_Ou) {//先手玉が動いた場合
				...
				sum.p[0][0] = 0;
				sum.p[1][0] = idlist.material * FVScale; sum.p[1][1] = 0;
				...
				for (int i = 0; i < EvalList::EvalListSize; i++) {
					...
					for (int j = 0; j < i; j++) {
						...
						sum.p[0][0] += ppskpp[index0_i][index0_j];
					}
					sum.p[1] += pkkp[index0_i];
				}
				sum.p[2][0] = KK[skpos][gkpos][0]; sum.p[2][1] = KK[skpos][gkpos][1];
			}
				...
				for (int i = 0; i < EvalList::EvalListSize; i++) {
					...
					for (int j = 0; j < i; j++) {
						...
						sum.p[0][1] += ppgkpp[index1_i][index1_j];
					}
					const auto index0_i = idlist.list0[i];
					sum.p[1] += pkkpi[index0_i];
				}
				sum.p[2][0] = KK[skpos][gkpos][0]; sum.p[2][1] = KK[skpos][gkpos][1];
			}
			else if (cap != Koma::None) {//玉以外の駒が動いて駒をとっていた場合
				Koma k = before.getKoma(from);
				...
				if (move.promote()) {
					...
					sum.p[1][0] += material_diff * FVScale;
				}
				EvalIndex aIndex0 = komaToIndex(k) + to;
				EvalIndex aIndex1 = komaToIndex(sgInv(k)) + inverse(to);
				...
				const unsigned gkpos = before.gOuPos();
				{
					...
					for (int i = 0; i < EvalList::EvalListSize; i++) {
						...
						sum.p[0][0] -= pskpp[index0];
						sum.p[0][1] -= pgkpp[index1];
						sum.p[0][0] -= cpskpp[index0];
						sum.p[0][1] -= cpgkpp[index1];
					}
					sum.p[0][0] += KPP[skpos][bIndex0][bcIndex0];//二重に引き算している部分を戻す
					sum.p[0][1] += KPP[inverse(gkpos)][bIndex1][bcIndex1];
					sum.p[1] -= KKP[skpos][gkpos][bIndex0];
					sum.p[1] -= KKP[skpos][gkpos][bcIndex0];
				}
				Replace(bIndex0, aIndex0, idlist.list0);
				Replace(bIndex1, aIndex1, idlist.list1);
				Replace(bcIndex0, acIndex0, idlist.list0);
				...
				}
				{
					...
					sum.p[0][0] -= KPP[skpos][aIndex0][acIndex0];//二重に足し算している部分を戻す
					sum.p[0][1] -= KPP[inverse(gkpos)][aIndex1][acIndex1];
					sum.p[1] += KKP[skpos][gkpos][aIndex0];
					sum.p[1] += KKP[skpos][gkpos][acIndex0];
				}
			}
...
#undef Replace
	...
	void kpp_kkpt_feat::recede(const Kyokumen& before, const koma::Koma moved, const koma::Koma captured, const Move move, const EvalSum& cache) {
		using namespace koma;
...
#undef Replace
	...
	bool kpp_kkpt_feat::operator==(const kpp_kkpt_feat& rhs)const {
	...
	std::string kpp_kkpt_feat::toString()const {
		std::string str;
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 2; j++) {
